...
def dp_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S,radiative,delta_source_1,delta_source_2,k_xp):
...
def dq_dt(q,u,p,K, N,t,M_inv,M_inv_S,radiative,delta_source_1,delta_source_2,k_xp,potential):
    dp_dx = np.empty_like(p)
    for k in range(K):
        dp_dx[k] = dp_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S, radiative,delta_source_1,delta_source_2,k_xp)
    return dp_dx
def dq_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S,radiative,delta_source_1,delta_source_2,k_xp):
    ...
    if k == k_xp:
        dq_dx_element +=  (delta_source_1(t)+delta_source_2(t))*M_inv[:,N]/2

    if k == k_xp+1:
        dq_dx_element += (delta_source_1(t)-delta_source_2(t))*M_inv[:,0]/2

    return dq_dx_element

def dp_dt(p,u,q,K, N,t,M_inv,M_inv_S,radiative,delta_source_1, delta_source_2,k_xp,potential):
    dq_dx = np.empty_like(q)
    for k in range(K):
        dq_dx[k] = dq_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S, radiative,delta_source_1, delta_source_2,k_xp)
    try:
        dq_dx += -u*potential
    except TypeError:
        pass
    return dq_dx
def du_dt(u,p,q,K,N,t,M_inv,M_inv_S,radiative,delta_source_1, delta_source_2, k_xp,potential):
    return p


def RK4_Step(dt, F,u,p,q, K, N,t, M_inv, M_inv_S, radiative = False,delta_source_1 = False, delta_source_2 = False, k_xp = False, potential = False):
    w1 = F(u            ,p,q, K, N, t         , M_inv, M_inv_S, radiative,delta_source_1, delta_source_2,k_xp,potential)
    w2 = F(u + 0.5*dt*w1,p,q, K, N, t + 0.5*dt, M_inv, M_inv_S, radiative,delta_source_1, delta_source_2,k_xp,potential)
    w3 = F(u + 0.5*dt*w2,p,q, K, N, t + 0.5*dt, M_inv, M_inv_S, radiative,delta_source_1, delta_source_2,k_xp,potential)
    w4 = F(u + dt*w3    ,p,q, K, N, t + dt    , M_inv, M_inv_S, radiative,delta_source_1, delta_source_2,k_xp,potential)
