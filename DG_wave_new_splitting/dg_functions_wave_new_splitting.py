...
def dp_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S,radiative):
    ...
    right_flux  =-M_inv[:,N] * (p[k%K][N]  - numerical_flux_p(p,q,k+1,K,N,t,radiative)) #flux on the right at x_{k+1}
    left_flux   = M_inv[:,0] * (p[k%K][0]  - numerical_flux_p(p,q,k  ,K,N,t,radiative))   #flux on the left  at x_{k}
    dp_dx_element = main_d_dx + left_flux + right_flux
    return dp_dx_element_k
...
def dq_dx_element_k(p,q,k,K,N,t,M_inv,M_inv_S,radiative):
    ...
    right_flux  =-M_inv[:,N] * (q[k%K][N]  - numerical_flux_q(p,q,k+1,K,N,t,radiative)) # at x_{k+1}
    left_flux   = M_inv[:,0] * (q[k%K][0]  - numerical_flux_q(p,q,k  ,K,N,t,radiative)) # at x_{k}
    dq_dx_element = main_d_dx + left_flux + right_flux
    return dq_dx_element_k
