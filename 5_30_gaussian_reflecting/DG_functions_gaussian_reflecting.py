...
def Evolve(t_initial, t_final, Tstepper, F,CF, start, end, initial_value_function, K, N,a,alpha,g= None,un = None):
    h = (end-start)/K
...
def du_dt_element_k_radiative(u,k, K, N,t,a,alpha, M_inv, M_inv_S, delta_source = None,un = None):
    ...
    if a > 0: 
        a=a
        if k == K-1:
           du_dt_element = first_term + third_term #+ second_term
           #du_dt_element[-1] = 0
        if k == 0:
           du_dt_element = first_term + second_term
    else:
        """if k == 0: #set flux at left end for v
            try:
                third_term = -M_inv[:,0] * (a*u[k][0]   - np.matmul(M_inv_S, un[0])[0])
                du_dt_element = first_term + second_term + third_term
            except TypeError:
                pass"""
        if k == K-1:
            try:
                ...
                du_dt_element = first_term + second_term + third_term 
                #du_dt_element[-1] -= np.matmul(M_inv_S, un[-1])[-1]
            except TypeError:
                pass
        if k == 0:
            du_dt_element = first_term + second_term
...
def DG_du_dt_radiative(u, K, N,t,a,alpha, M_inv, M_inv_S,delta_source = None, v = None, potential = None, un = None):
    ...
    for k in range(K):
        du_dt_elements[k] = du_dt_element_k_radiative(u,k, K, N,t,a,alpha, M_inv, M_inv_S,delta_source,un)
    
    if a > 0:
        try:
            du_dt_elements += v + potential
...
def RK4_Step(dt, F,u, K, N,t,a,alpha, M_inv, M_inv_S, g = None,v= None, potential = None,un = None):
    w1 = F(u            , K, N, t         ,a, alpha, M_inv, M_inv_S, g,v,potential, un)
    w2 = F(u + 0.5*dt*w1, K, N, t + 0.5*dt,a, alpha, M_inv, M_inv_S, g,v,potential, un)
    w3 = F(u + 0.5*dt*w2, K, N, t + 0.5*dt,a, alpha, M_inv, M_inv_S, g,v,potential, un)
    w4 = F(u + dt*w3    , K, N, t + dt    ,a, alpha, M_inv, M_inv_S, g,v,potential, un)
