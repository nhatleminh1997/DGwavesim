...
# F is the derivative
def RK4_Step(dt, F,u, K, N,t,a,alpha, M_inv, M_inv_S, g = None,v= None):
    w1 = F(u            , K, N, t         ,a, alpha, M_inv, M_inv_S, g,v)
    w2 = F(u + 0.5*dt*w1, K, N, t + 0.5*dt,a, alpha, M_inv, M_inv_S, g,v)
    w3 = F(u + 0.5*dt*w2, K, N, t + 0.5*dt,a, alpha, M_inv, M_inv_S, g,v)
    w4 = F(u + dt*w3    , K, N, t + dt    ,a, alpha, M_inv, M_inv_S, g,v)
...
def DG_du_dt_radiative(u, K, N,t,a,alpha, M_inv, M_inv_S,delta_source = None, v = None):
    du_dt_elements = np.empty_like(u)
    for k in range(K):
