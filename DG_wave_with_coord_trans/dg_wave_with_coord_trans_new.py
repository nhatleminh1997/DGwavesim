...
def numerical_flux_go_with_p(p,q, k, K, N, t, radiative,x_p_dot):
    g = x_p_dot(t) + 1
    h = x_p_dot(t) - 1
    flux = g**2*h*q[k%K][0] - g*h**2*q[(k-1)%K][N] + h**2*p[(k-1)%K][N] - g**2*p[(k)%K][0]   #      WORKING VERSION
...
def numerical_flux_go_with_q(p,q, k, K, N, t, radiative,x_p_dot):
    g = x_p_dot(t) + 1
    h = x_p_dot(t) - 1
    flux = h*p[(k-1)%K][N] - g*p[k%K][0] + g*h*q[k%K][0]  - g*h*q[(k-1)%K][N]                   #   WORKING VERSION
    if radiative == True:
        if k == 0:
                flux = -p[0][0]   #of course wrong
        if k == K:
                flux = -p[-1][-1] #of course wrong
    flux = flux/2
